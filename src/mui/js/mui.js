...
(function($, window, undefined) {
	...
	var serialize = function(params, obj, traditional, scope) {
		...
		$.each(obj, function(key, value) {
			...
			// handle data.json in serializeArray() format
			if(!scope && array) {
	...
	$.get = function( /* url, data.json, success, dataType */ ) {
		return $.ajax(parseArguments.apply(null, arguments));
	};

	$.post = function( /* url, data.json, success, dataType */ ) {
		var options = parseArguments.apply(null, arguments);
		options.type = 'POST';
		return $.ajax(options);
	};

	$.getJSON = function( /* url, data.json, success */ ) {
		var options = parseArguments.apply(null, arguments);
...
(function($, window, document, undefined) {
	...
	var Scroll = $.Class.extend({
		...
		destroy: function() {
			...
			delete $.data[this.wrapper.getAttribute('data.json-scroll')];
			this.wrapper.setAttribute('data.json-scroll', '');
		}
	});
	//Indicator
	...
	$.fn.scroll = function(options) {
		...
		this.each(function() {
			var scrollApi = null;
			var self = this;
			var id = self.getAttribute('data.json-scroll');
			if (!id) {
				id = ++$.uuid;
				...
				self.setAttribute('data.json-scroll', id);
			} else {
				scrollApi = $.data[id];
			}
...
(function($, window, document, undefined) {
	...
	$.fn.pullRefresh = function(options) {
		if (this.length === 1) {
			...
			var id = self.getAttribute('data.json-pullrefresh');
			...
			if (!id) {
				...
				self.setAttribute('data.json-pullrefresh', id);
			} else {
...
(function($, window) {
	...
	var Slider = $.Slider = $.Scroll.extend({
		...
		_handleSlide: function(e) {
			...
			if (indicatorWrap) {
				if (indicatorWrap.getAttribute('data.json-scroll')) { //scroll
		...
		_drag: function(e) {
			...
			if (direction === 'left' || direction === 'right') {
				//拖拽期间取消定时
				var slidershowTimer = this.wrapper.getAttribute('data.json-slidershowTimer');
		...
		_initTimer: function() {
			...
			var slidershowTimer = slider.getAttribute('data.json-slidershowTimer');
			slidershowTimer && window.clearTimeout(slidershowTimer);
			if (interval) {
				...
				slider.setAttribute('data.json-slidershowTimer', slidershowTimer);
			}
		},
		...
		destroy: function() {
			this._initEvent(true); //detach
			delete $.data[this.wrapper.getAttribute('data.json-slider')];
			this.wrapper.setAttribute('data.json-slider', '');
		}
	});
	$.fn.slider = function(options) {
		...
		this.each(function() {
			...
			if (sliderElement && sliderElement.querySelector(SELECTOR_SLIDER_ITEM)) {
				var id = sliderElement.getAttribute('data.json-slider');
				if (!id) {
					id = ++$.uuid;
					$.data[id] = slider = new Slider(sliderElement, options);
					sliderElement.setAttribute('data.json-slider', id);
				} else {
					slider = $.data[id];
					if (slider && options) {
...
(function($, document) {
    ...
    $.plusReady(function() {
            ...
            _initPulldownRefreshEvent: function() {
                ...
                $.plusReady(function() {
                        ...
                        webview.addEventListener('close', function() {
                            ...
                            self.element.removeAttribute('data.json-pullrefresh-plus-' + attrWebviewId);
                        });
        ...
        $.fn.pullRefresh_native = function(options) {
            ...
            var id = self.getAttribute('data.json-pullrefresh-plus-' + attrWebviewId);
            if (!id && typeof args === 'undefined') {
                return false;
            }
            if (!id) { //避免重复初始化5+ pullrefresh
                id = ++$.uuid;
                self.setAttribute('data.json-pullrefresh-plus-' + attrWebviewId, id);
...
(function($, window, document, name) {
	...
	$.fn.offCanvas = function(options) {
		...
		this.each(function() {
			...
			var id = self.getAttribute('data.json-offCanvas');
			if (!id) {
				id = ++$.uuid;
				$.data[id] = offCanvasApi = new OffCanvas(self, options);
				self.setAttribute('data.json-offCanvas', id);
			} else {
...
(function($, window, name) {
	...
	$.fn['switch'] = function(options) {
		...
		this.each(function() {
			var switchApi = null;
			var id = this.getAttribute('data.json-switch');
			if (!id) {
				id = ++$.uuid;
				$.data[id] = new Toggle(this);
				this.setAttribute('data.json-switch', id);
			} else {
				switchApi = $.data[id];
			}
...
(function($, window, document) {
	...
	$.fn.input = function(options) {
		...
		this.each(function() {
			...
			var id = this.getAttribute('data.json-input-' + actions[0]);
			if (!id) {
				id = ++$.uuid;
				...
				for (var i = 0, len = actions.length; i < len; i++) {
					this.setAttribute('data.json-input-' + actions[i], id);
				}
			} else {
				inputApi = $.data[id];
			}
...
(function($, window) {
    ...
    $.fn.transparent = function(options) {
        ...
        this.each(function() {
            ...
            if (!transparentApi) {
                var top = this.getAttribute('data.json-top');
                var offset = this.getAttribute('data.json-offset');
                var duration = this.getAttribute('data.json-duration');
                var scrollby = this.getAttribute('data.json-scrollby');
    ...
    $.fn.numbox = function(options) {
        ...
        this.each(function(i, element) {
            ...
            if (options) {
                ...
                var optionsText = element.getAttribute('data.json-numbox-options');
                var options = optionsText ? JSON.parse(optionsText) : {};
                options.step = element.getAttribute('data.json-numbox-step') || options.step;
                options.min = element.getAttribute('data.json-numbox-min') || options.min;
                options.max = element.getAttribute('data.json-numbox-max') || options.max;
                element.numbox = new Numbox(element, options);
            }
        });
...
(function($, window, document) {
    ...
    $.fn.button = function(state) {
        ...
        this.each(function() {
            ...
            if (!buttonApi) {
                var loadingText = this.getAttribute('data.json-loading-text');
                var loadingIcon = this.getAttribute('data.json-loading-icon');
                var loadingIconPosition = this.getAttribute('data.json-loading-icon-position');
